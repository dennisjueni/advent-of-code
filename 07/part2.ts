...
const solve = () => {
  ...
  let currentSpace = rootDir.fileSize;
  let avlblSpace = totalSpace - currentSpace;

  let spaceToFree = neededSpace - avlblSpace;

  let currentMinDirectory = Infinity;
  ...
  const traverse = (dir: Dir) => {
    if (dir.fileSize >= spaceToFree && dir.fileSize < currentMinDirectory) {
      currentMinDirectory = dir.fileSize;
    }
  ...
  console.log(currentMinDirectory);
};
